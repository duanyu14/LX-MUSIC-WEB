...
const BUILTIN_SOURCES = [
    {
        id: 'ikun-builtin.js',
        name: 'ikun音源[内置][永久]',
        sourceUrl: 'https://c.wwwweb.top/script/lxmusic?key=IKM-P92200001-pfXun0urVy1iOd33-Jz',
        enabled: true
    }
]

// 下载内置音源
async function downloadBuiltinSource(sourceUrl: string, savePath: string) {
    return new Promise<void>((resolve, reject) => {
        needle.get(sourceUrl, (err, resp) => {
            if (err) {
                reject(err)
                return
            }
            ...
            fs.writeFileSync(savePath, resp.body.toString())
            resolve()
        })
    })
}
...
// 初始化内置音源
async function initBuiltinSources(sourceRoot: string) {
    const openDir = path.join(sourceRoot, '_open')
    const sourcesJsonPath = path.join(openDir, 'sources.json')
    
    // 检查是否需要初始化
    if (fs.existsSync(sourcesJsonPath)) {
        try {
            const existingSources = JSON.parse(fs.readFileSync(sourcesJsonPath, 'utf-8'))
            const hasBuiltin = existingSources.some((s: any) => s.id === 'ikun-builtin.js')
            if (hasBuiltin) {
                console.log('[UserApi] 内置音源已存在，跳过初始化')
                return
            }
        } catch (e) {
            // sources.json 解析失败，继续初始化
        }
    }
    ...
    for (const builtin of BUILTIN_SOURCES) {
        const scriptPath = path.join(openDir, builtin.id)
        
        if (!fs.existsSync(scriptPath)) {
            console.log(`[UserApi] 正在下载内置音源: ${builtin.name}`)
            try {
                await downloadBuiltinSource(builtin.sourceUrl, scriptPath)
                console.log(`[UserApi] ✓ 内置音源下载成功: ${builtin.name}`)
            } catch (error: any) {
                console.error(`[UserApi] ✗ 内置音源下载失败: ${builtin.name}`, error.message)
                continue
            }
        }
        ...
        if (fs.existsSync(sourcesJsonPath)) {
            try {
                sources = JSON.parse(fs.readFileSync(sourcesJsonPath, 'utf-8'))
            } catch (e) {
                sources = []
            }
        }
        ...
        // 检查是否已存在
        const existingIndex = sources.findIndex((s: any) => s.id === builtin.id)
        ...
        const sourceInfo = {
            id: builtin.id,
            name: metadata.name || builtin.name,
            version: metadata.version || '1.0.0',
            author: metadata.author || '',
            description: metadata.description || '',
            homepage: metadata.homepage || '',
            size: script.length,
            supportedSources: [], // 会在加载时自动更新
        ...
        if (existingIndex >= 0) {
            sources[existingIndex] = sourceInfo
        } else {
            sources.unshift(sourceInfo) // 内置音源放在最前面
        }
        
        fs.writeFileSync(sourcesJsonPath, JSON.stringify(sources, null, 2))
        console.log(`[UserApi] 内置音源已添加到配置: ${builtin.name}`)
    }
}
...
export async function initUserApis(targetUser?: string) {
    ...
    if (!targetUser) {
        try {
            await initBuiltinSources(sourceRoot)
        } catch (error: any) {
            console.error('[UserApi] 初始化内置音源失败:', error.message)
        }
    }
